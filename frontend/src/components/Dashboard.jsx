...
const Dashboard = () => {
  ...
  const handleDeleteAllRooms = async () => {
    if (rooms.length === 0) {
        alert("No active rooms to delete.");
        return;
    }
    if(window.confirm(`Are you sure you want to end all ${rooms.length} active sessions?`)) {
        try {
            // End each room one by one
            await Promise.all(rooms.map(room => roomAPI.endRoom(room.roomId)));
            fetchRooms();
            alert("All active rooms ended successfully");
        } catch (error) {
            console.error("Error ending one or more rooms:", error);
            alert("Failed to end all rooms. Check console for details.");
        }
    }
  };
  ...
  return (
    <div 
        className="min-h-screen text-white font-sans selection:bg-gray-700"
        style={{
            backgroundColor: 'rgba(0, 0, 0, 0.9)', // Fallback and dark overlay
            backgroundImage: `url(${backgroundImageURL})`,
            backgroundSize: 'cover',
            backgroundPosition: 'center',
            backgroundAttachment: 'fixed',
            backgroundBlendMode: 'overlay', // Helps keep text readable
        }}
    >
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-24 pb-12">
        ...
        <div className="flex flex-col gap-6">
          ...
          {/* 1. Share Your Code (First Row, Full Width) */}
          <div className="bg-[#111]/90 border border-[#222] rounded-3xl p-8 flex flex-col gap-8 backdrop-blur-sm">
              ...
              <div className="flex flex-col md:flex-row items-center justify-between gap-8 border-b border-[#222] pb-8">
                ...
                <div className="w-full md:w-auto flex-shrink-0">
                  ...
                  >
                      {loading ? (
                          <div className="animate-spin w-5 h-5 mx-auto border-2 border-black border-t-transparent rounded-full"></div>
                      ) : (
                          "Start Meeting"
                      )}
                  </button>
          ...
          {/* Second Row: 2. Active Rooms (Left) and 3. Statistics (Right) */}
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
            
            {/* 2. Active Rooms (Left Column) */}
            <div className="lg:col-span-8 flex flex-col gap-6">
              {/* Added /90 opacity and backdrop-blur-sm for better readability against the background image */}
              <div className="bg-[#111]/90 border border-[#222] rounded-3xl p-6 md:p-8 flex flex-col gap-4 backdrop-blur-sm"> 
              
                <div className="flex-1 flex flex-col">
                  <div className="flex justify-between items-center mb-6">
                      <h2 className="text-gray-400 text-sm uppercase tracking-wider font-bold">Active Rooms</h2>
                      
                      <button onClick={handleDeleteAllRooms} className="text-xs text-red-500 hover:text-red-300 transition-colors font-bold">
                          Delete All
                      </button>
                  </div>
            ...
            {/* 3. Meeting Analytics (Right Column) */}
            <div className="lg:col-span-4 flex flex-col gap-6">
                {/* Added /90 opacity and backdrop-blur-sm for better readability against the background image */}
                <div className="bg-[#111]/90 border border-[#222] rounded-3xl p-8 min-h-[230px] backdrop-blur-sm"> 
